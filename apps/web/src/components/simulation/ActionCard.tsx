...
export default function ActionCard({
  ...
  return (
    ...
    >
      ...
      {/* Always-visible Execute button (fast-play path at default intensity) */}
      {!isExpanded && (
        <button
          className={`btn btn--sm w-full${side === "red" ? " btn--danger" : " btn--primary"}${executed ? " btn--success" : ""}`}
          onClick={handleExecute}
          disabled={isSubmitting || executed}
          style={{ marginTop: "0.4rem" }}
        >
          {executed ? "✓ Executed" : `Execute (${tier.label})`}
        </button>
      )}
        ...
        <>
          ...
          <div className="action-card__controls" style={{ marginTop: "0.5rem" }}>
            <button
              ...
              style={{ width: "100%" }}
            >
              {executed ? "✓ Executed" : isSubmitting ? "..." : `Execute (${tier.label})`}
            </button>
          </div>
        </>
      )}
